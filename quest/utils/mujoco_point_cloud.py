...
class PointCloudGenerator(object):
    ...
    def generate_cropped_point_cloud(self, save_img_dir=None, device_id=0):
        o3d_clouds = []
        cam_poses = []
        depths = []
        for cam_i in range(len(self.cam_names)):
            # Render and optionally save image from camera corresponding to cam_i
            color_img, depth = self.capture_image(self.cam_names[cam_i], capture_depth=True, device_id=device_id)
            depths.append(depth)
            ...
            # todo: this code does not work and i don't know who wrote it
    ...
    def capture_image(self, camera_name, capture_depth=True, device_id=0):
        ...
        if capture_depth:
            ...
            depth_convert = self.depth_img_2_meters(depth)
